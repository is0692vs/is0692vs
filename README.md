<!-- weather-greeting:start -->
# 🌍 Olá! Dia nublado☁️
📍 Lisbon: 29°C
<!-- weather-greeting:end -->

<!-- vscode-stats:start -->
🚀 VSCode Extensions:
- **[Jules Extension](https://github.com/is0692vs/jules-extension)**: 6,057 installs | ⭐ 4.5/5 (2 ratings) | v1.5.5
- **[Code Mantra](https://github.com/is0692vs/code-mantra)**: 15 installs | ⭐ No ratings yet | v1.1.9
- **[Link Canvas](https://github.com/is0692vs/link-canvas)**: 62 installs | ⭐ No ratings yet | v0.1.3

![VSCode Extension Stats](https://quickchart.io/chart?c=%7B%22type%22%3A%22line%22%2C%22data%22%3A%7B%22labels%22%3A%5B%222026-05-22%22%2C%222026-05-23%22%2C%222026-05-24%22%2C%222026-05-25%22%2C%222026-05-26%22%2C%222026-05-27%22%2C%222026-05-28%22%2C%222026-05-29%22%2C%222026-05-30%22%2C%222026-05-31%22%2C%222026-06-01%22%2C%222026-06-02%22%2C%222026-06-03%22%2C%222026-06-04%22%2C%222026-06-05%22%2C%222026-06-06%22%2C%222026-06-07%22%2C%222026-06-08%22%2C%222026-06-09%22%2C%222026-06-10%22%2C%222026-06-11%22%2C%222026-06-12%22%2C%222026-06-13%22%2C%222026-06-14%22%2C%222026-06-15%22%2C%222026-06-16%22%2C%222026-06-17%22%2C%222026-06-18%22%2C%222026-06-19%22%2C%222026-06-20%22%5D%2C%22datasets%22%3A%5B%7B%22label%22%3A%22Jules%20Extension%22%2C%22data%22%3A%5B5445%2C5457%2C5483%2C5508%2C5534%2C5552%2C5567%2C5588%2C5600%2C5617%2C5650%2C5675%2C5698%2C5728%2C5737%2C5771%2C5791%2C5814%2C5848%2C5868%2C5883%2C5909%2C5915%2C5941%2C5958%2C5979%2C6005%2C6021%2C6040%2C6057%5D%2C%22borderColor%22%3A%22%239966FF%22%2C%22backgroundColor%22%3A%22transparent%22%2C%22tension%22%3A0.4%7D%2C%7B%22label%22%3A%22Code%20Mantra%22%2C%22data%22%3A%5B15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%5D%2C%22borderColor%22%3A%22%23FF9F40%22%2C%22backgroundColor%22%3A%22transparent%22%2C%22tension%22%3A0.4%7D%2C%7B%22label%22%3A%22Link%20Canvas%22%2C%22data%22%3A%5B56%2C56%2C56%2C56%2C58%2C58%2C58%2C58%2C58%2C59%2C59%2C59%2C59%2C59%2C59%2C61%2C61%2C61%2C61%2C61%2C61%2C61%2C61%2C61%2C61%2C61%2C61%2C61%2C62%2C62%5D%2C%22borderColor%22%3A%22%23FF6384%22%2C%22backgroundColor%22%3A%22transparent%22%2C%22tension%22%3A0.4%7D%5D%7D%2C%22options%22%3A%7B%22title%22%3A%7B%22display%22%3Atrue%2C%22text%22%3A%22VSCode%20Extension%20Installs%22%7D%2C%22scales%22%3A%7B%22yAxes%22%3A%5B%7B%22ticks%22%3A%7B%22beginAtZero%22%3Atrue%7D%7D%5D%7D%7D%7D&width=800&height=400)
<!-- vscode-stats:end -->

<!-- stats:start -->

_npm download statistics temporarily unavailable_

<!-- stats:end -->

<!-- active-projects:start -->
## 🔨 Active Projects (Last 30 Days)

### 🤖 gemini2.5flashによる直近30日の活動サマリー

直近30日間の活動サマリー:
素晴らしい30日間でしたね！なんと合計241件ものコミット、本当に精力的に開発を進めていますね！🚀

特に「OpenShelf」では、目覚ましい活躍が見られます！
セキュリティ脆弱性の修正でシステムの安全性を高めたり、トークンキャッシュの効率化やタグのカウント・重複排除ロジックの最適化によってパフォーマンスを大幅に改善したりと、サービス品質向上に大きく貢献しています。
他にも、単体テストの追加やリファクタリングでコード品質を向上させつつ、定期的なリリースも着実に実施しており、安定した運用と改善の両立を実現していますね！✨

「jules-extension」では、aria-disabled属性の修正に取り組んでいましたね。細かな部分にも気を配り、品質向上に努める姿勢、素晴らしいです！💡

多岐にわたる活動、本当にお疲れ様でした。チームへの大きな貢献に感謝します！これからもその勢いで頑張っていきましょう！💪

_Total: 233 commits across 10 projects_

<table>
  <tr>
    <td align="center" width="33%">
      <a href="https://github.com/Hiroki-org/OpenShelf" target="_blank">
        <img src="https://opengraph.githubassets.com/1/Hiroki-org/OpenShelf" alt="OpenShelf" width="100%" />
      </a>
      <br />
      <sub><strong><a href="https://github.com/Hiroki-org/OpenShelf" target="_blank">OpenShelf</a></strong></sub>
      <br />
      <sub>📊 196 commits (84.1%)</sub>
      <br />
      <sub>🔷 TypeScript </sub>
    </td>
    <td align="center" width="33%">
      <a href="https://github.com/Hiroki-org/jules-extension" target="_blank">
        <img src="https://opengraph.githubassets.com/1/Hiroki-org/jules-extension" alt="jules-extension" width="100%" />
      </a>
      <br />
      <sub><strong><a href="https://github.com/Hiroki-org/jules-extension" target="_blank">jules-extension</a></strong></sub>
      <br />
      <sub>📊 34 commits (14.6%)</sub>
      <br />
      <sub>🔷 TypeScript ⭐ 16</sub>
    </td>
    <td align="center" width="33%">
      <a href="https://github.com/Hiroki-org/Audicle" target="_blank">
        <img src="https://opengraph.githubassets.com/1/Hiroki-org/Audicle" alt="Audicle" width="100%" />
      </a>
      <br />
      <sub><strong><a href="https://github.com/Hiroki-org/Audicle" target="_blank">Audicle</a></strong></sub>
      <br />
      <sub>📊 3 commits (1.3%)</sub>
      <br />
      <sub>🔷 TypeScript </sub>
    </td>
  </tr>
</table>

<!-- active-projects:end -->

<!-- spotify:start -->
## 🎵 My Top Tracks on Spotify (Recently)

<table>
  <tr>
    <td align="center">
      <a href="https://open.spotify.com/track/1ycsBZlPqRPqf0ilosVhEl" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b273f46c070998294b66650ea007" alt="盛れ！ミ・アモーレ" width="120" />
      </a>
      <br />
      <sub><strong>#1</strong></sub>
      <br />
      <sub>盛れ！ミ・アモーレ</sub>
      <br />
      <sub>Juice=Juice</sub>
    </td>
    <td align="center">
      <a href="https://open.spotify.com/track/3BX1ySyWmklDJISRDbHnpS" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b273e41e9af294137bc4e23bdfe8" alt="盛れ！ミ・アモーレ - Concert 2025 Queen of Hearts Special Flush" width="120" />
      </a>
      <br />
      <sub><strong>#2</strong></sub>
      <br />
      <sub>盛れ！ミ・アモーレ - Concert 2025 Queen of Hearts Special Flush</sub>
      <br />
      <sub>Juice=Juice</sub>
    </td>
    <td align="center">
      <a href="https://open.spotify.com/track/4UKANDuD17KGukGIp5g0Cx" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b2736f91c9419b51f070bbeefc41" alt="C.U.R.I.O.S.I.T.Y. (feat. Paledusk and CHICO CARLITO)" width="120" />
      </a>
      <br />
      <sub><strong>#3</strong></sub>
      <br />
      <sub>C.U.R.I.O.S.I.T.Y. (feat. Paledusk and CHICO CARLITO)</sub>
      <br />
      <sub>ONE OK ROCK, Paledusk, CHICO CARLITO</sub>
    </td>
    <td align="center">
      <a href="https://open.spotify.com/track/1sbEsttOqyGIYDVN5Uuoy7" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b273aa582122453603bedbdc4cec" alt="飯伏幸太のテーマ｢Golden Star｣" width="120" />
      </a>
      <br />
      <sub><strong>#4</strong></sub>
      <br />
      <sub>飯伏幸太のテーマ｢Golden Star｣</sub>
      <br />
      <sub>NJPW</sub>
    </td>
    <td align="center">
      <a href="https://open.spotify.com/track/2S3u18aJF8eExll5dYwSJb" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b2734b0ed3119e27ff6e56298053" alt="RATATATA" width="120" />
      </a>
      <br />
      <sub><strong>#5</strong></sub>
      <br />
      <sub>RATATATA</sub>
      <br />
      <sub>BABYMETAL, Electric Callboy</sub>
    </td>
  </tr>
</table>
<!-- spotify:end -->

<!-- github-releases:start -->
## 🚀 Recent Releases

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.5.5-publish">v1.5.5-publish</a> · 1 weeks ago
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.5.5-publish -->
## What's Changed
### ✨ Features
* 🎨 Palette: [UX improvement] セッションラベルにツールチップを追加し、省略されたテキストを確認可能にする by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/630
### 🐛 Bug Fixes

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.5.5-publish)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.5.5">v1.5.5</a> · 1 weeks ago
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.5.5 -->
## What's Changed
### ✨ Features
* 🎨 Palette: [UX improvement] セッションラベルにツールチップを追加し、省略されたテキストを確認可能にする by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/630
### 🐛 Bug Fixes
### 📦 Dependencies

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.5.5)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.5.4-publish">v1.5.4-publish</a> · 3 weeks ago
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.5.4-publish -->
## What's Changed
### ✨ Features
* 🎨 Palette: sessionLabel の省略表示に対するツールチップと aria-label の追加 by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/616
### 🐛 Bug Fixes

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.5.4-publish)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.5.4">v1.5.4</a> · 3 weeks ago
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.5.4 -->
## What's Changed
### ✨ Features
* 🎨 Palette: sessionLabel の省略表示に対するツールチップと aria-label の追加 by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/616
### 🐛 Bug Fixes

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.5.4)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.5.3">v1.5.3</a> · May 20, 2026
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.5.3 -->
## What's Changed
### ✨ Features
* ⚡ Bolt: [performance improvement] Use Map to avoid Array.find in fetchAndCheckoutFromPRInfo by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/608

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.5.3)
</details>
<!-- github-releases:end -->

---
