<!-- weather-greeting:start -->
# 🌍 G'day! Rainy weather☔
📍 Sydney: 16°C
<!-- weather-greeting:end -->

<!-- vscode-stats:start -->
🚀 VSCode Extensions:
- **[Jules Extension](https://github.com/is0692vs/jules-extension)**: 7,362 installs | ⭐ 4.5/5 (2 ratings) | v1.6.1
- **[Code Mantra](https://github.com/is0692vs/code-mantra)**: 17 installs | ⭐ No ratings yet | v1.1.9
- **[Link Canvas](https://github.com/is0692vs/link-canvas)**: 75 installs | ⭐ No ratings yet | v0.1.3

![VSCode Extension Stats](https://quickchart.io/chart?c=%7B%22type%22%3A%22line%22%2C%22data%22%3A%7B%22labels%22%3A%5B%222026-08-28%22%2C%222026-08-29%22%2C%222026-08-30%22%2C%222026-08-31%22%2C%222026-09-01%22%2C%222026-09-02%22%2C%222026-09-03%22%2C%222026-09-04%22%2C%222026-09-05%22%2C%222026-09-06%22%2C%222026-09-07%22%2C%222026-09-08%22%2C%222026-09-09%22%2C%222026-09-10%22%2C%222026-09-11%22%2C%222026-09-12%22%2C%222026-09-13%22%2C%222026-09-14%22%2C%222026-09-15%22%2C%222026-09-16%22%2C%222026-09-17%22%2C%222026-09-18%22%2C%222026-09-19%22%2C%222026-09-20%22%2C%222026-09-21%22%2C%222026-09-22%22%2C%222026-09-23%22%2C%222026-09-24%22%2C%222026-09-25%22%2C%222026-09-26%22%5D%2C%22datasets%22%3A%5B%7B%22label%22%3A%22Jules%20Extension%22%2C%22data%22%3A%5B6998%2C7003%2C7019%2C7029%2C7041%2C7064%2C7075%2C7085%2C7097%2C7112%2C7125%2C7138%2C7152%2C7166%2C7172%2C7185%2C7192%2C7195%2C7219%2C7238%2C7249%2C7256%2C7282%2C7292%2C7306%2C7316%2C7330%2C7338%2C7354%2C7362%5D%2C%22borderColor%22%3A%22%239966FF%22%2C%22backgroundColor%22%3A%22transparent%22%2C%22tension%22%3A0.4%7D%2C%7B%22label%22%3A%22Code%20Mantra%22%2C%22data%22%3A%5B16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C17%2C17%2C17%2C17%2C17%5D%2C%22borderColor%22%3A%22%23FF9F40%22%2C%22backgroundColor%22%3A%22transparent%22%2C%22tension%22%3A0.4%7D%2C%7B%22label%22%3A%22Link%20Canvas%22%2C%22data%22%3A%5B71%2C71%2C71%2C71%2C71%2C71%2C71%2C72%2C72%2C72%2C72%2C72%2C72%2C72%2C73%2C73%2C73%2C73%2C74%2C74%2C74%2C74%2C74%2C74%2C74%2C75%2C75%2C75%2C75%2C75%5D%2C%22borderColor%22%3A%22%23FF6384%22%2C%22backgroundColor%22%3A%22transparent%22%2C%22tension%22%3A0.4%7D%5D%7D%2C%22options%22%3A%7B%22title%22%3A%7B%22display%22%3Atrue%2C%22text%22%3A%22VSCode%20Extension%20Installs%22%7D%2C%22scales%22%3A%7B%22yAxes%22%3A%5B%7B%22ticks%22%3A%7B%22beginAtZero%22%3Atrue%7D%7D%5D%7D%7D%7D&width=800&height=400)
<!-- vscode-stats:end -->

<!-- stats:start -->

_npm download statistics temporarily unavailable_

<!-- stats:end -->

<!-- active-projects:start -->
## 🔨 Active Projects (Last 30 Days)

### 🤖 gemini2.5flashによる直近30日の活動サマリー

直近30日間の活動サマリー:
直近30日間で、133件ものコミット、お疲れ様でした！👏 複数のプロジェクトで活発な開発が進んだ素晴らしい期間でしたね。

特に注目すべき活動は以下の通りです。

**Audicle**では、protobufjsやundici、sharp、next、nanoidなどの複数の依存関係を精力的に更新し、システムの安定性と最新の状態を保つための基盤固めに大きく貢献しました。`dependabot`や`chore(deps)`系のコミットが目立ち、堅実なメンテナンス作業が光りますね！✨

**OpenShelf**では、非常に多岐にわたる重要な改善活動を展開しました。セキュリティ脆弱性のある依存関係の更新、クロスプラットフォーム対応、アクセシビリティ改善（例: `palette-toast-aria-role`、`pdf-viewer-a11y`）といったユーザー体験向上に直結する修正が多く見られました。また、`Release: staging -> main`などのブランチマージやリリース準備も迅速に対応し、プロジェクトの推進に貢献しました。セキュリティ強化と品質向上への取り組み、素晴らしいです！🛡️

**paper-tools**でも、`npm_and_yarn`グループの依存関係を更新し、プロジェクトの健全性を維持してくれました。

日々の細かい修正から大規模な依存関係の更新、セキュリティ強化まで、幅広いタスクをこなし、プロジェクト全体を着実に前進させていますね。引き続きの活躍を楽しみにしています！💪

_Total: 132 commits across 6 projects_

<table>
  <tr>
    <td align="center" width="33%">
      <a href="https://github.com/Hiroki-org/Audicle" target="_blank">
        <img src="https://opengraph.githubassets.com/1/Hiroki-org/Audicle" alt="Audicle" width="100%" />
      </a>
      <br />
      <sub><strong><a href="https://github.com/Hiroki-org/Audicle" target="_blank">Audicle</a></strong></sub>
      <br />
      <sub>📊 56 commits (42.4%)</sub>
      <br />
      <sub>🔷 TypeScript </sub>
    </td>
    <td align="center" width="33%">
      <a href="https://github.com/Hiroki-org/OpenShelf" target="_blank">
        <img src="https://opengraph.githubassets.com/1/Hiroki-org/OpenShelf" alt="OpenShelf" width="100%" />
      </a>
      <br />
      <sub><strong><a href="https://github.com/Hiroki-org/OpenShelf" target="_blank">OpenShelf</a></strong></sub>
      <br />
      <sub>📊 31 commits (23.5%)</sub>
      <br />
      <sub>🔷 TypeScript </sub>
    </td>
    <td align="center" width="33%">
      <a href="https://github.com/Hiroki-org/paper-tools" target="_blank">
        <img src="https://opengraph.githubassets.com/1/Hiroki-org/paper-tools" alt="paper-tools" width="100%" />
      </a>
      <br />
      <sub><strong><a href="https://github.com/Hiroki-org/paper-tools" target="_blank">paper-tools</a></strong></sub>
      <br />
      <sub>📊 21 commits (15.9%)</sub>
      <br />
      <sub>🔷 TypeScript </sub>
    </td>
  </tr>
</table>

<!-- active-projects:end -->

<!-- spotify:start -->
## 🎵 My Top Tracks on Spotify (Recently)

<table>
  <tr>
    <td align="center">
      <a href="https://open.spotify.com/track/5So15vlWHcIFmBNbSnewks" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b27394fb3480c556174040915928" alt="War Dogs" width="120" />
      </a>
      <br />
      <sub><strong>#1</strong></sub>
      <br />
      <sub>War Dogs</sub>
      <br />
      <sub>[Q]brick, Kensei Abbot</sub>
    </td>
    <td align="center">
      <a href="https://open.spotify.com/track/1sbEsttOqyGIYDVN5Uuoy7" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b273aa582122453603bedbdc4cec" alt="飯伏幸太のテーマ｢Golden Star｣" width="120" />
      </a>
      <br />
      <sub><strong>#2</strong></sub>
      <br />
      <sub>飯伏幸太のテーマ｢Golden Star｣</sub>
      <br />
      <sub>NJPW</sub>
    </td>
    <td align="center">
      <a href="https://open.spotify.com/track/3xUQAdNRIJIZwMaABAW30d" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b2734cd6430c81db387bb7bed830" alt="飯伏幸太のテーマ 「Golden☆Star 2020ver.」" width="120" />
      </a>
      <br />
      <sub><strong>#3</strong></sub>
      <br />
      <sub>飯伏幸太のテーマ 「Golden☆Star 2020ver.」</sub>
      <br />
      <sub>NJPW(新日本プロレスリング株式会社)</sub>
    </td>
    <td align="center">
      <a href="https://open.spotify.com/track/7jwgLHy2EMK9vcbGdlFp99" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b273b098ce3dd24e07d4519dd8f9" alt="Battle Cry (Kenny Omega Theme)" width="120" />
      </a>
      <br />
      <sub><strong>#4</strong></sub>
      <br />
      <sub>Battle Cry (Kenny Omega Theme)</sub>
      <br />
      <sub>All Elite Wrestling</sub>
    </td>
    <td align="center">
      <a href="https://open.spotify.com/track/01zwFLoQgAISCdEJqTTwbt" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b27370e97e2585eafe060cd05acf" alt="Love Sick" width="120" />
      </a>
      <br />
      <sub><strong>#5</strong></sub>
      <br />
      <sub>Love Sick</sub>
      <br />
      <sub>AiNA THE END</sub>
    </td>
  </tr>
</table>
<!-- spotify:end -->

<!-- github-releases:start -->
## 🚀 Recent Releases

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.6.1-publish">v1.6.1-publish</a> · Yesterday
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.6.1-publish -->
## What's Changed
### ✨ Features
* 🎨 Palette: 非同期処理中のキャンセルボタンの無効化状態の説明を追加 by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/776
### 🔧 Other Changes

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.6.1-publish)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.6.1">v1.6.1</a> · Yesterday
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.6.1 -->
## What's Changed
### ✨ Features
* 🎨 Palette: 非同期処理中のキャンセルボタンの無効化状態の説明を追加 by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/776
### 🔧 Other Changes

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.6.1)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.6.0">v1.6.0</a> · Jul 30, 2026
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.6.0 -->
## What's Changed
### 👷 CI/CD
* chore: prepare release 1.6.0 by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/774
### 📦 Dependencies

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.6.0)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.6.0-publish">v1.6.0-publish</a> · Jul 30, 2026
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.6.0-publish -->
## What's Changed
### 👷 CI/CD
* chore: prepare release 1.6.0 by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/774
### 📦 Dependencies

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.6.0-publish)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.5.7">v1.5.7</a> · Jul 28, 2026
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.5.7 -->
## What's Changed
### ✨ Features
* 🎨 Palette: コピーボタンのツールチップとARIAラベルの同期 by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/723
### 📦 Dependencies
### 🔧 Other Changes

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.5.7)
</details>
<!-- github-releases:end -->

---
