<!-- weather-greeting:start -->
# 🌍 Hello! Beautiful sunny day☀️
📍 New York: 15°C
<!-- weather-greeting:end -->

<!-- vscode-stats:start -->
🚀 VSCode Extensions:
- **[Jules Extension](https://github.com/is0692vs/jules-extension)**: 4,695 installs | ⭐ 4.5/5 (2 ratings) | v1.4.2
- **[Code Mantra](https://github.com/is0692vs/code-mantra)**: 15 installs | ⭐ No ratings yet | v1.1.9
- **[Link Canvas](https://github.com/is0692vs/link-canvas)**: 53 installs | ⭐ No ratings yet | v0.1.3

![VSCode Extension Stats](https://quickchart.io/chart?c=%7B%22type%22%3A%22line%22%2C%22data%22%3A%7B%22labels%22%3A%5B%222026-04-02%22%2C%222026-04-03%22%2C%222026-04-04%22%2C%222026-04-05%22%2C%222026-04-06%22%2C%222026-04-07%22%2C%222026-04-08%22%2C%222026-04-09%22%2C%222026-04-10%22%2C%222026-04-11%22%2C%222026-04-12%22%2C%222026-04-13%22%2C%222026-04-14%22%2C%222026-04-15%22%2C%222026-04-16%22%2C%222026-04-17%22%2C%222026-04-18%22%2C%222026-04-19%22%2C%222026-04-20%22%2C%222026-04-21%22%2C%222026-04-22%22%2C%222026-04-23%22%2C%222026-04-24%22%2C%222026-04-25%22%2C%222026-04-26%22%2C%222026-04-27%22%2C%222026-04-28%22%2C%222026-04-29%22%2C%222026-04-30%22%2C%222026-05-01%22%5D%2C%22datasets%22%3A%5B%7B%22label%22%3A%22Jules%20Extension%22%2C%22data%22%3A%5B4077%2C4093%2C4108%2C4122%2C4140%2C4157%2C4172%2C4198%2C4219%2C4234%2C4252%2C4285%2C4308%2C4339%2C4363%2C4393%2C4407%2C4418%2C4452%2C4470%2C4498%2C4520%2C4541%2C4566%2C4593%2C4610%2C4617%2C4638%2C4660%2C4695%5D%2C%22borderColor%22%3A%22%239966FF%22%2C%22backgroundColor%22%3A%22transparent%22%2C%22tension%22%3A0.4%7D%2C%7B%22label%22%3A%22Code%20Mantra%22%2C%22data%22%3A%5B12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C14%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%5D%2C%22borderColor%22%3A%22%23FF9F40%22%2C%22backgroundColor%22%3A%22transparent%22%2C%22tension%22%3A0.4%7D%2C%7B%22label%22%3A%22Link%20Canvas%22%2C%22data%22%3A%5B44%2C44%2C44%2C44%2C44%2C44%2C44%2C44%2C44%2C44%2C44%2C45%2C46%2C47%2C47%2C48%2C48%2C49%2C49%2C49%2C49%2C50%2C50%2C50%2C50%2C52%2C52%2C52%2C53%2C53%5D%2C%22borderColor%22%3A%22%23FF6384%22%2C%22backgroundColor%22%3A%22transparent%22%2C%22tension%22%3A0.4%7D%5D%7D%2C%22options%22%3A%7B%22title%22%3A%7B%22display%22%3Atrue%2C%22text%22%3A%22VSCode%20Extension%20Installs%22%7D%2C%22scales%22%3A%7B%22yAxes%22%3A%5B%7B%22ticks%22%3A%7B%22beginAtZero%22%3Atrue%7D%7D%5D%7D%7D%7D&width=800&height=400)
<!-- vscode-stats:end -->

<!-- stats:start -->

_npm download statistics temporarily unavailable_

<!-- stats:end -->

<!-- active-projects:start -->
## 🔨 Active Projects (Last 30 Days)

### 🤖 gemini2.5flashによる直近30日の活動サマリー

直近30日間の活動サマリー:
直近30日間で、なんと472件ものコミット、素晴らしいですね！🚀 その精力的な活動を分析したサマリーです。

**OpenShelf**では、複数回にわたる`staging`から`main`へのリリースやブランチマージを精力的に行い、主要なコードベースの管理とリリースサイクルを支えました。また、`drizzle-kit`のバージョンアップによる依存関係の更新や、JSONボディの型定義を明確化するコード健全性向上にも貢献していますね！✨

**paper-tools**では、DOI解決の堅牢化や「Save to Notion」の使用などコード健全性の向上に取り組むと共に、プロパティ検索の最適化やレコメンダーの並行処理制限によるパフォーマンス改善、DBLP著者検索のテスト追加など、品質とパフォーマンスの両面で大きく前進させました。

**Audicle**では、`RemoteAudioSynthesizer`のユニットテストを追加し、テストカバレッジを向上させています。🧪

**jules-extension**では、Codecovバンドル解析の導入でビルドプロセスを強化し、不要なファイルのクリーンアップも行いましたね。🧹

**github-user-summary**では、キャッシュ関連の改善にマージコミットが見られました。

多岐にわたるプロジェクトで、着実な進捗と品質向上に貢献されていることがよく分かります。今後のさらなるご活躍を期待しています！👏

_Total: 831 commits across 10 projects_

<table>
  <tr>
    <td align="center" width="33%">
      <a href="https://github.com/Hiroki-org/OpenShelf" target="_blank">
        <img src="https://opengraph.githubassets.com/1/Hiroki-org/OpenShelf" alt="OpenShelf" width="100%" />
      </a>
      <br />
      <sub><strong><a href="https://github.com/Hiroki-org/OpenShelf" target="_blank">OpenShelf</a></strong></sub>
      <br />
      <sub>📊 468 commits (56.3%)</sub>
      <br />
      <sub>🔷 TypeScript </sub>
    </td>
    <td align="center" width="33%">
      <a href="https://github.com/Hiroki-org/jules-extension" target="_blank">
        <img src="https://opengraph.githubassets.com/1/Hiroki-org/jules-extension" alt="jules-extension" width="100%" />
      </a>
      <br />
      <sub><strong><a href="https://github.com/Hiroki-org/jules-extension" target="_blank">jules-extension</a></strong></sub>
      <br />
      <sub>📊 157 commits (18.9%)</sub>
      <br />
      <sub>🔷 TypeScript ⭐ 14</sub>
    </td>
    <td align="center" width="33%">
      <a href="https://github.com/Hiroki-org/github-user-summary" target="_blank">
        <img src="https://opengraph.githubassets.com/1/Hiroki-org/github-user-summary" alt="github-user-summary" width="100%" />
      </a>
      <br />
      <sub><strong><a href="https://github.com/Hiroki-org/github-user-summary" target="_blank">github-user-summary</a></strong></sub>
      <br />
      <sub>📊 135 commits (16.2%)</sub>
      <br />
      <sub>🔷 TypeScript </sub>
    </td>
  </tr>
</table>

<!-- active-projects:end -->

<!-- spotify:start -->
## 🎵 My Top Tracks on Spotify (Recently)

<table>
  <tr>
    <td align="center">
      <a href="https://open.spotify.com/track/4UKANDuD17KGukGIp5g0Cx" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b2736f91c9419b51f070bbeefc41" alt="C.U.R.I.O.S.I.T.Y. (feat. Paledusk and CHICO CARLITO)" width="120" />
      </a>
      <br />
      <sub><strong>#1</strong></sub>
      <br />
      <sub>C.U.R.I.O.S.I.T.Y. (feat. Paledusk and CHICO CARLITO)</sub>
      <br />
      <sub>ONE OK ROCK, Paledusk, CHICO CARLITO</sub>
    </td>
    <td align="center">
      <a href="https://open.spotify.com/track/6jlGKRWFDRDs7nEhzLaN5K" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b273b8a3cb5f89cce62885de71ed" alt="章" width="120" />
      </a>
      <br />
      <sub><strong>#2</strong></sub>
      <br />
      <sub>章</sub>
      <br />
      <sub>Ryokuoushoku Shakai</sub>
    </td>
    <td align="center">
      <a href="https://open.spotify.com/track/6zO9kSYSVed2iQQSOrKUGQ" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b273cf68803dfbc2fbb83d97eb67" alt="STARDUST～内藤哲也のテーマ" width="120" />
      </a>
      <br />
      <sub><strong>#3</strong></sub>
      <br />
      <sub>STARDUST～内藤哲也のテーマ</sub>
      <br />
      <sub>KAZSIN</sub>
    </td>
    <td align="center">
      <a href="https://open.spotify.com/track/2S3u18aJF8eExll5dYwSJb" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b2734b0ed3119e27ff6e56298053" alt="RATATATA" width="120" />
      </a>
      <br />
      <sub><strong>#4</strong></sub>
      <br />
      <sub>RATATATA</sub>
      <br />
      <sub>BABYMETAL, Electric Callboy</sub>
    </td>
    <td align="center">
      <a href="https://open.spotify.com/track/6AGHSgBeirk7AHXWOM3vKp" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b27328995b924c172c40d17d690c" alt="Nothing." width="120" />
      </a>
      <br />
      <sub><strong>#5</strong></sub>
      <br />
      <sub>Nothing.</sub>
      <br />
      <sub>BiSH</sub>
    </td>
  </tr>
</table>
<!-- spotify:end -->

<!-- github-releases:start -->
## 🚀 Recent Releases

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.4.2">v1.4.2</a> · Today
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.4.2 -->
## What's Changed
### ✨ Features
* Optimize performance by avoiding Set recreation in loop by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/508
### 🐛 Bug Fixes
### 📦 Dependencies

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.4.2)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.4.2-publish">v1.4.2-publish</a> · Today
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.4.2-publish -->
## What's Changed
### ✨ Features
* Optimize performance by avoiding Set recreation in loop by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/508
### 🐛 Bug Fixes
### 📦 Dependencies

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.4.2-publish)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.4.1">v1.4.1</a> · 5 days ago
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.4.1 -->
## What's Changed
### ✨ Features
* build: add codecov bundle analysis by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/491
### 📦 Dependencies
### 🔧 Other Changes

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.4.1)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.4.1-publish">v1.4.1-publish</a> · 5 days ago
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.4.1-publish -->
## What's Changed
### ✨ Features
* build: add codecov bundle analysis by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/491
### 📦 Dependencies
### 🔧 Other Changes

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.4.1-publish)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.4.0-publish">v1.4.0-publish</a> · 6 days ago
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.4.0-publish -->
## What's Changed
### ✨ Features
* ⚡ Optimize GitHubAuth.getToken() to use cache directly by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/481
### 👷 CI/CD
### 🔧 Other Changes

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.4.0-publish)
</details>
<!-- github-releases:end -->

---
