<!-- weather-greeting:start -->
# 🌍 ¡Hola! Día nublado☁️
📍 Lima: 20°C
<!-- weather-greeting:end -->

<!-- vscode-stats:start -->
🚀 VSCode Extensions:
- **[Jules Extension](https://github.com/is0692vs/jules-extension)**: 4,583 installs | ⭐ 4.5/5 (2 ratings) | v1.4.1
- **[Code Mantra](https://github.com/is0692vs/code-mantra)**: 15 installs | ⭐ No ratings yet | v1.1.9
- **[Link Canvas](https://github.com/is0692vs/link-canvas)**: 50 installs | ⭐ No ratings yet | v0.1.3

![VSCode Extension Stats](https://quickchart.io/chart?c=%7B%22type%22%3A%22line%22%2C%22data%22%3A%7B%22labels%22%3A%5B%222026-03-28%22%2C%222026-03-29%22%2C%222026-03-30%22%2C%222026-03-31%22%2C%222026-04-01%22%2C%222026-04-02%22%2C%222026-04-03%22%2C%222026-04-04%22%2C%222026-04-05%22%2C%222026-04-06%22%2C%222026-04-07%22%2C%222026-04-08%22%2C%222026-04-09%22%2C%222026-04-10%22%2C%222026-04-11%22%2C%222026-04-12%22%2C%222026-04-13%22%2C%222026-04-14%22%2C%222026-04-15%22%2C%222026-04-16%22%2C%222026-04-17%22%2C%222026-04-18%22%2C%222026-04-19%22%2C%222026-04-20%22%2C%222026-04-21%22%2C%222026-04-22%22%2C%222026-04-23%22%2C%222026-04-24%22%2C%222026-04-25%22%2C%222026-04-26%22%5D%2C%22datasets%22%3A%5B%7B%22label%22%3A%22Jules%20Extension%22%2C%22data%22%3A%5B4002%2C4024%2C4033%2C4053%2C4072%2C4077%2C4093%2C4108%2C4122%2C4140%2C4157%2C4172%2C4198%2C4219%2C4234%2C4252%2C4285%2C4308%2C4339%2C4363%2C4393%2C4407%2C4418%2C4452%2C4470%2C4498%2C4520%2C4541%2C4566%2C4583%5D%2C%22borderColor%22%3A%22%239966FF%22%2C%22backgroundColor%22%3A%22transparent%22%2C%22tension%22%3A0.4%7D%2C%7B%22label%22%3A%22Code%20Mantra%22%2C%22data%22%3A%5B12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C12%2C14%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%5D%2C%22borderColor%22%3A%22%23FF9F40%22%2C%22backgroundColor%22%3A%22transparent%22%2C%22tension%22%3A0.4%7D%2C%7B%22label%22%3A%22Link%20Canvas%22%2C%22data%22%3A%5B44%2C44%2C44%2C44%2C44%2C44%2C44%2C44%2C44%2C44%2C44%2C44%2C44%2C44%2C44%2C44%2C45%2C46%2C47%2C47%2C48%2C48%2C49%2C49%2C49%2C49%2C50%2C50%2C50%2C50%5D%2C%22borderColor%22%3A%22%23FF6384%22%2C%22backgroundColor%22%3A%22transparent%22%2C%22tension%22%3A0.4%7D%5D%7D%2C%22options%22%3A%7B%22title%22%3A%7B%22display%22%3Atrue%2C%22text%22%3A%22VSCode%20Extension%20Installs%22%7D%2C%22scales%22%3A%7B%22yAxes%22%3A%5B%7B%22ticks%22%3A%7B%22beginAtZero%22%3Atrue%7D%7D%5D%7D%7D%7D&width=800&height=400)
<!-- vscode-stats:end -->

<!-- stats:start -->

_npm download statistics temporarily unavailable_

<!-- stats:end -->

<!-- active-projects:start -->
## 🔨 Active Projects (Last 30 Days)

### 🤖 gemini2.5flashによる直近30日の活動サマリー

直近30日間の活動サマリー:
直近30日間で**489件**ものコミット、素晴らしいペースですね！👏

まず**OpenShelf**では、stagingブランチからの大規模なマージや、依存関係（`drizzle-kit`など）のアップデートを通じて、プロジェクトの安定化と最新化に大きく貢献しました。多数のコンフリクトを解決しながら、堅牢な基盤を築いているのがわかります！

**Audicle**では、Chrome Extensionの`RemoteAudioSynthesizer`に単体テストを追加し、テストカバレッジと信頼性を大幅に向上させましたね。品質向上への努力、お見事です！🧪

**jules-extension**では、`codecov bundle analysis`の追加や、不要な`package-lock.json`やローカルアーティファクトの削除、パフォーマンス最適化のためのマージなど、効率化とクリーンアップに大きく貢献しました。アセットの抽出やバージョンアップも行い、プロジェクトの健全性を高めています。🚀

**paper-tools**では、メインブランチをコード健全性向上、パフォーマンス最適化、テスト追加など、多岐にわたる開発ブランチに取り込む作業が多く見られました。これは、プロジェクト全体で継続的な改善が進んでいる証拠ですね！

どのプロジェクトも着実に進化しており、チームの皆さんの活躍が目に見えてわかります。この調子でこれからも素晴らしい開発を続けてください！✨

_Total: 1030 commits across 10 projects_

<table>
  <tr>
    <td align="center" width="33%">
      <a href="https://github.com/Hiroki-org/OpenShelf" target="_blank">
        <img src="https://opengraph.githubassets.com/1/Hiroki-org/OpenShelf" alt="OpenShelf" width="100%" />
      </a>
      <br />
      <sub><strong><a href="https://github.com/Hiroki-org/OpenShelf" target="_blank">OpenShelf</a></strong></sub>
      <br />
      <sub>📊 491 commits (47.7%)</sub>
      <br />
      <sub>🔷 TypeScript </sub>
    </td>
    <td align="center" width="33%">
      <a href="https://github.com/Hiroki-org/paper-tools" target="_blank">
        <img src="https://opengraph.githubassets.com/1/Hiroki-org/paper-tools" alt="paper-tools" width="100%" />
      </a>
      <br />
      <sub><strong><a href="https://github.com/Hiroki-org/paper-tools" target="_blank">paper-tools</a></strong></sub>
      <br />
      <sub>📊 201 commits (19.5%)</sub>
      <br />
      <sub>🔷 TypeScript </sub>
    </td>
    <td align="center" width="33%">
      <a href="https://github.com/Hiroki-org/jules-extension" target="_blank">
        <img src="https://opengraph.githubassets.com/1/Hiroki-org/jules-extension" alt="jules-extension" width="100%" />
      </a>
      <br />
      <sub><strong><a href="https://github.com/Hiroki-org/jules-extension" target="_blank">jules-extension</a></strong></sub>
      <br />
      <sub>📊 162 commits (15.7%)</sub>
      <br />
      <sub>🔷 TypeScript ⭐ 14</sub>
    </td>
  </tr>
</table>

<!-- active-projects:end -->

<!-- spotify:start -->
## 🎵 My Top Tracks on Spotify (Recently)

<table>
  <tr>
    <td align="center">
      <a href="https://open.spotify.com/track/4UKANDuD17KGukGIp5g0Cx" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b2736f91c9419b51f070bbeefc41" alt="C.U.R.I.O.S.I.T.Y. (feat. Paledusk and CHICO CARLITO)" width="120" />
      </a>
      <br />
      <sub><strong>#1</strong></sub>
      <br />
      <sub>C.U.R.I.O.S.I.T.Y. (feat. Paledusk and CHICO CARLITO)</sub>
      <br />
      <sub>ONE OK ROCK, Paledusk, CHICO CARLITO</sub>
    </td>
    <td align="center">
      <a href="https://open.spotify.com/track/5BtCZAPpqRW74K9Fme7IE6" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b2736fe3a6609a96447bd8c1e09d" alt="Primitive" width="120" />
      </a>
      <br />
      <sub><strong>#2</strong></sub>
      <br />
      <sub>Primitive</sub>
      <br />
      <sub>BiSH</sub>
    </td>
    <td align="center">
      <a href="https://open.spotify.com/track/6AGHSgBeirk7AHXWOM3vKp" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b27328995b924c172c40d17d690c" alt="Nothing." width="120" />
      </a>
      <br />
      <sub><strong>#3</strong></sub>
      <br />
      <sub>Nothing.</sub>
      <br />
      <sub>BiSH</sub>
    </td>
    <td align="center">
      <a href="https://open.spotify.com/track/6jlGKRWFDRDs7nEhzLaN5K" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b273b8a3cb5f89cce62885de71ed" alt="章" width="120" />
      </a>
      <br />
      <sub><strong>#4</strong></sub>
      <br />
      <sub>章</sub>
      <br />
      <sub>Ryokuoushoku Shakai</sub>
    </td>
    <td align="center">
      <a href="https://open.spotify.com/track/2iFegucLSbIncYLOJVxnSU" target="_blank">
        <img src="https://i.scdn.co/image/ab67616d0000b27331de4bcc9f38137474313dd4" alt="絶対アイドル辞めないで" width="120" />
      </a>
      <br />
      <sub><strong>#5</strong></sub>
      <br />
      <sub>絶対アイドル辞めないで</sub>
      <br />
      <sub>=LOVE</sub>
    </td>
  </tr>
</table>
<!-- spotify:end -->

<!-- github-releases:start -->
## 🚀 Recent Releases

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.4.1">v1.4.1</a> · Today
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.4.1 -->
## What's Changed
### ✨ Features
* build: add codecov bundle analysis by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/491
### 📦 Dependencies
### 🔧 Other Changes

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.4.1)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.4.1-publish">v1.4.1-publish</a> · Today
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.4.1-publish -->
## What's Changed
### ✨ Features
* build: add codecov bundle analysis by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/491
### 📦 Dependencies
### 🔧 Other Changes

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.4.1-publish)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.4.0-publish">v1.4.0-publish</a> · Today
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.4.0-publish -->
## What's Changed
### ✨ Features
* ⚡ Optimize GitHubAuth.getToken() to use cache directly by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/481
### 👷 CI/CD
### 🔧 Other Changes

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.4.0-publish)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.4.0">v1.4.0</a> · Today
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.4.0 -->
## What's Changed
### ✨ Features
* ⚡ Optimize GitHubAuth.getToken() to use cache directly by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/481
### 👷 CI/CD
### 🔧 Other Changes

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.4.0)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.3.11-publish">v1.3.11-publish</a> · 2 days ago
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.3.11-publish -->
## What's Changed
### ✨ Features
* ⚡ Optimize GitHubAuth.getToken to use cache by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/472
### 🐛 Bug Fixes
### 🔧 Other Changes

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.3.11-publish)
</details>
<!-- github-releases:end -->

---
