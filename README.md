<!-- weather-greeting:start -->
# 🌍 おはよう！雨ですね☔
📍 Hiroshima: 28°C
<!-- weather-greeting:end -->

<!-- vscode-stats:start -->
🚀 VSCode Extensions:
- **[Jules Extension](https://github.com/is0692vs/jules-extension)**: 6,927 installs | ⭐ 4.5/5 (2 ratings) | v1.6.0
- **[Code Mantra](https://github.com/is0692vs/code-mantra)**: 16 installs | ⭐ No ratings yet | v1.1.9
- **[Link Canvas](https://github.com/is0692vs/link-canvas)**: 69 installs | ⭐ No ratings yet | v0.1.3

![VSCode Extension Stats](https://quickchart.io/chart?c=%7B%22type%22%3A%22line%22%2C%22data%22%3A%7B%22labels%22%3A%5B%222026-07-23%22%2C%222026-07-24%22%2C%222026-07-25%22%2C%222026-07-26%22%2C%222026-07-27%22%2C%222026-07-28%22%2C%222026-07-29%22%2C%222026-07-30%22%2C%222026-07-31%22%2C%222026-08-01%22%2C%222026-08-02%22%2C%222026-08-03%22%2C%222026-08-04%22%2C%222026-08-05%22%2C%222026-08-06%22%2C%222026-08-07%22%2C%222026-08-08%22%2C%222026-08-09%22%2C%222026-08-10%22%2C%222026-08-11%22%2C%222026-08-12%22%2C%222026-08-13%22%2C%222026-08-14%22%2C%222026-08-15%22%2C%222026-08-16%22%2C%222026-08-17%22%2C%222026-08-18%22%2C%222026-08-19%22%2C%222026-08-20%22%2C%222026-08-21%22%5D%2C%22datasets%22%3A%5B%7B%22label%22%3A%22Jules%20Extension%22%2C%22data%22%3A%5B6573%2C6583%2C6600%2C6616%2C6624%2C6644%2C6648%2C6657%2C6676%2C6687%2C6704%2C6715%2C6720%2C6745%2C6763%2C6768%2C6778%2C6788%2C6790%2C6805%2C6818%2C6826%2C6848%2C6852%2C6863%2C6881%2C6893%2C6906%2C6927%2C6927%5D%2C%22borderColor%22%3A%22%239966FF%22%2C%22backgroundColor%22%3A%22transparent%22%2C%22tension%22%3A0.4%7D%2C%7B%22label%22%3A%22Code%20Mantra%22%2C%22data%22%3A%5B15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C15%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%2C16%5D%2C%22borderColor%22%3A%22%23FF9F40%22%2C%22backgroundColor%22%3A%22transparent%22%2C%22tension%22%3A0.4%7D%2C%7B%22label%22%3A%22Link%20Canvas%22%2C%22data%22%3A%5B65%2C65%2C65%2C65%2C65%2C65%2C65%2C65%2C65%2C65%2C65%2C65%2C65%2C66%2C66%2C67%2C68%2C68%2C69%2C69%2C69%2C69%2C69%2C69%2C69%2C69%2C69%2C69%2C69%2C69%5D%2C%22borderColor%22%3A%22%23FF6384%22%2C%22backgroundColor%22%3A%22transparent%22%2C%22tension%22%3A0.4%7D%5D%7D%2C%22options%22%3A%7B%22title%22%3A%7B%22display%22%3Atrue%2C%22text%22%3A%22VSCode%20Extension%20Installs%22%7D%2C%22scales%22%3A%7B%22yAxes%22%3A%5B%7B%22ticks%22%3A%7B%22beginAtZero%22%3Atrue%7D%7D%5D%7D%7D%7D&width=800&height=400)
<!-- vscode-stats:end -->

<!-- stats:start -->

_npm download statistics temporarily unavailable_

<!-- stats:end -->

<!-- active-projects:start -->
## 🔨 Active Projects (Last 30 Days)

### 🤖 gemini2.5flashによる直近30日の活動サマリー

直近30日間の活動サマリー:
直近30日間で合計**127件**もの素晴らしいコミット、お疲れ様でした！✨ 様々なプロジェクトで精力的に活動されましたね。

特に**OpenShelf**では、依存関係のセキュリティ脆弱性修正や、Paletteのキーボードナビゲーション改善（フォーカスリング追加）でアクセシビリティ向上に大きく貢献されました。さらに、コード健全性のためのリファクタリングや、staging/mainへの多数のマージ対応でプロジェクトの安定性と品質維持に尽力されています。

**jules-extension**では、Node.js 20への要件更新を行い、開発環境の基盤をしっかりと整えましたね。

**github-user-summary**では、`BusinessCard`コンポーネントを細分化する大規模なリファクタリングを実施し、コードの保守性と再利用性を高めました。素晴らしい改善です！🧹

そして**Audicle**では、`buildQueueWithLegacySystem`のリファクタリングや、プレイリストアイテム検索の最適化を進め、コードの可読性向上とパフォーマンス改善に貢献されました。

どのプロジェクトでも、セキュリティ、アクセシビリティ、コード品質、パフォーマンス向上に取り組む姿勢が伺え、大変素晴らしい成果です！この調子でこれからも頑張っていきましょう！🚀

_Total: 127 commits across 6 projects_

<table>
  <tr>
    <td align="center" width="33%">
      <a href="https://github.com/Hiroki-org/OpenShelf" target="_blank">
        <img src="https://opengraph.githubassets.com/1/Hiroki-org/OpenShelf" alt="OpenShelf" width="100%" />
      </a>
      <br />
      <sub><strong><a href="https://github.com/Hiroki-org/OpenShelf" target="_blank">OpenShelf</a></strong></sub>
      <br />
      <sub>📊 48 commits (37.8%)</sub>
      <br />
      <sub>🔷 TypeScript </sub>
    </td>
    <td align="center" width="33%">
      <a href="https://github.com/Hiroki-org/jules-extension" target="_blank">
        <img src="https://opengraph.githubassets.com/1/Hiroki-org/jules-extension" alt="jules-extension" width="100%" />
      </a>
      <br />
      <sub><strong><a href="https://github.com/Hiroki-org/jules-extension" target="_blank">jules-extension</a></strong></sub>
      <br />
      <sub>📊 40 commits (31.5%)</sub>
      <br />
      <sub>🔷 TypeScript ⭐ 19</sub>
    </td>
    <td align="center" width="33%">
      <a href="https://github.com/Hiroki-org/paper-tools" target="_blank">
        <img src="https://opengraph.githubassets.com/1/Hiroki-org/paper-tools" alt="paper-tools" width="100%" />
      </a>
      <br />
      <sub><strong><a href="https://github.com/Hiroki-org/paper-tools" target="_blank">paper-tools</a></strong></sub>
      <br />
      <sub>📊 15 commits (11.8%)</sub>
      <br />
      <sub>🔷 TypeScript </sub>
    </td>
  </tr>
</table>

<!-- active-projects:end -->

<!-- spotify:start -->
🎵 My Top Tracks on Spotify (Recently):

_Error fetching tracks. Please try again later._
<!-- spotify:end -->

<!-- github-releases:start -->
## 🚀 Recent Releases

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.6.0">v1.6.0</a> · 3 weeks ago
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.6.0 -->
## What's Changed
### 👷 CI/CD
* chore: prepare release 1.6.0 by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/774
### 📦 Dependencies

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.6.0)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.6.0-publish">v1.6.0-publish</a> · 3 weeks ago
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.6.0-publish -->
## What's Changed
### 👷 CI/CD
* chore: prepare release 1.6.0 by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/774
### 📦 Dependencies

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.6.0-publish)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.5.7">v1.5.7</a> · 3 weeks ago
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.5.7 -->
## What's Changed
### ✨ Features
* 🎨 Palette: コピーボタンのツールチップとARIAラベルの同期 by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/723
### 📦 Dependencies
### 🔧 Other Changes

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.5.7)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.5.7-publish">v1.5.7-publish</a> · 3 weeks ago
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.5.7-publish -->
## What's Changed
### ✨ Features
* 🎨 Palette: コピーボタンのツールチップとARIAラベルの同期 by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/723
### 📦 Dependencies
### 🔧 Other Changes

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.5.7-publish)
</details>

<details>
<summary>
<strong>Jules Extension</strong> <a href="https://github.com/Hiroki-org/jules-extension/releases/tag/v1.5.6-publish">v1.5.6-publish</a> · Jul 10, 2026
</summary>

<!-- Release notes generated using configuration in .github/release.yml at v1.5.6-publish -->
## What's Changed
### ✨ Features
* 🎨 Palette: 空状態コンテナへのスクリーンリーダー対応強化（aria-live追加） by @is0692vs in https://github.com/Hiroki-org/jules-extension/pull/667
### 🐛 Bug Fixes
### 📦 Dependencies

[View on GitHub](https://github.com/Hiroki-org/jules-extension/releases/tag/v1.5.6-publish)
</details>
<!-- github-releases:end -->

---
